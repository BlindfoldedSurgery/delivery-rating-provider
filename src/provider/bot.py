...
def get_restaurant_list_url(postal_code: str, *, base_url: str = "https://cw-api.takeaway.com",
                            limit: int = 0, is_accurate: bool = True, show_test_restaurants: bool = False) -> str:
    is_accurate_url: str = str(is_accurate).lower()
    show_test_restaurants_url: str = str(show_test_restaurants).lower()

    return (f"{base_url}/api/v33/restaurants?postalCode={postal_code}&limit={limit}"
...
def retrieve_restaurants(_url: str, *, timeout: int) -> list[RestaurantListItem]:
    ...
    return [RestaurantListItem.from_dict(restaurants[restaurant_key]) for restaurant_key in restaurants]


async def get_random_restaurants(url: str, *, count: int = 1, filter_fn: Optional[Callable[[Restaurant], bool]] = None,
                                 timeout: int = 15) -> list[Restaurant]:
    """
    ...
    if filter_fn is not None:
        restaurants_a = (Restaurant.from_list_item(list_item, timeout=timeout) for list_item in
                         list_items)
        restaurants = await asyncio.gather(*restaurants_a, return_exceptions=True)

        filtered_restaurants = [_restaurant for _restaurant in restaurants if
                                not isinstance(_restaurant, BaseException) and filter_fn(_restaurant)]

        for error in [restaurant for restaurant in restaurants if isinstance(restaurant, BaseException)]:
            log.exception(repr(error))

        chosen_restaurants = list(filtered_restaurants)
    else:
        chosen_restaurants = [await Restaurant.from_list_item(list_item) for list_item in list_items]
...
async def command_random(update: Update, context: ContextTypes.DEFAULT_TYPE, *,
    ...
    return await update.effective_message.reply_text(message, disable_web_page_preview=True,  # type: ignore
